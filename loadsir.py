...
def loadsir(filename,headonly=0):
    """loadsir
    function (image, head, descrip, iaopt)=loadsir('filename' <,headonly=1>)
    - or -
    function SIR=loadsir('filename' <,headonly=1>)

    load a sirf image and head array into python.  Also sets global variables
    ...
    headonly:   (optional)when present and equal to one, does not read image, just head
    """
    ...
    if idatatype==4:  # floating point file
        # open file again for separately reading floats
        fid2 = open(filename)
        # skip beginning of head
        fromfile(fid2,dtype=data_types,count=51,sep="")
        # read floating point values from file
...
# read image data
    ...
    if headonly!=1:
      if idatatype==1: # very rare
        ...
        image = flipud(reshape(im_in,(int(head[1]),int(head[0])),order='C'))  # read floating point data
